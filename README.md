...
By default, `uhppoted-cli` uses the communal `uhppoted.conf` configuration file shared by all the `uhppoted` project modules which is (_or will eventually be_) documented in [uhppoted](https://codeberg.org/uhppoted/uhppoted). 
...
```
uhppoted-cli --config ./uhppote.mine get-device 4726234734
```
...
The above commands build the `uhppoted-cli` executable to the `bin` directory.

## uhppoted-cli

Usage: ```uhppoted-cli [options] <command> <parameters>```
...
```
  ...
  Example:
   ...
   ./uhppoted-cli --debug --config ./uhppoted.local get-time 4156216363
```
...
```
uhppoted-cli get-device 405419896
uhppoted-cli get-device alpha
```
...
- `current date`
```
uhppoted-cli [options] get-devices
...
uhppoted-cli [options] get-devices
  ...
  uhppoted-cli --debug --conf ./405419896.conf --timeout 1.25s get-device 405419896
...
```
uhppoted-cli [options] set-address <device> <address> [mask] [gateway]
  ...
  uhppoted-cli set-address 405419896 192.168.1.49 255.255.255.0 192.168.1.1
```
...
- `time` _(HH:mm:ss)_
```
uhppoted-cli [options] get-time <device>
  ...
  uhppoted-cli get-time 405419896
...
- `time` _(HH:mm:ss)_
```
uhppoted-cli [options] set-time <device> [date/time]

  <device>      (required) Controller serial number
  <date/time>   Date/time in YYYY-MM-DD HH:mm:ss format or use 'now' to use the current time on the uhppoted-cli host). Defaults to 'now'.
  ...
  uhppoted-cli set-time 405419896 now
  uhppoted-cli set-time 405419896 2020-05-21 10:55:32
...
- `delay` _(seconds)_
```
uhppoted-cli [options] get-door-delay <device> <door>
  ...
  uhppoted-cli get-door-delay 405419896 3
...
uhppoted-cli [options] get-door-delay <device> <door>
  ...
  uhppoted-cli set-door-delay 405419896 3 5
...
```
uhppoted-cli [options] get-door-control <device> <door>
  ...
  uhppoted-cli get-door-control 405419896 3
...
- `control state` _(normally open/normally closed/controlled)_
```
uhppoted-cli [options] set-door-control <device> <door> <state>
  ...
  uhppoted-cli set-door-control 405419896 3 "normally open"
...
```
uhppoted-cli [options] get-listener <device>
  ...
  uhppoted-cli get-listener 405419896
...
```
uhppoted-cli [options] set-listener <device> <address:port> [interval]
  ...
  uhppoted-cli set-listener 405419896 192.168.1.100:60001
 
  405419896  true

  uhppoted-cli set-listener 405419896 192.168.1.100:60001 7
...
```
uhppoted-cli [options] record-special-events <device> <enable>
  ...
  Example:
  > uhppoted-cli record-special-events 405419896 false
    405419896  true
```
...
```
uhppoted-cli [options] get-status <device>
  ...
  uhppoted-cli get-status 405419896
...
```
uhppoted-cli [options] get-cards <device ID>
  ...
  uhppoted-cli get-cards 405419896
...
```
uhppoted-cli [options] get-card <device ID> <card number>
  ...
  1. uhppoted-cli get-card 405419896 10058400
  
     10568400  2021-01-01 2021-12-31 Y N Y 29 7531 1,4


  2. uhppoted-cli get-card 405419896 100584399
...
- `firstcard` _first-card privileges_
```
uhppoted-cli [options] put-card <device ID> <card number> <start> <end> <doors> [--firstcard <firstcard>] [--card-format <any|wiegand-26>]
  ...
  uhppoted-cli put-card 405419896 10058400 2023-01-01 2023-12-31 1,3,4:29 7531 --firstcard 1,4 --card-format any
  405419896 10058400 true

  uhppoted-cli put-card  --card-format wiegand-26 405419896 1058400 2023-01-01 2023-12-31 1,3,4:29 7531
  405419896 10058400 true
```
**NOTES**
1. For a door associated with a time profile, `uhppoted-cli` requires the time profile to be an existing time profile defined in the controller. The controller itself does not enforce this requirement but linking to a non-existent time profile counter-intuitively seems to allow access at any time of day.


#### `delete-card`
Unconditionally deletes an access card record from a controller, returning `true` if successful.
```
uhppoted-cli [options] delete-card <device ID> <card number>
  ...
  uhppoted-cli delete-card 405419896 8165538
...
```
uhppoted-cli [options] delete-all <device ID>
  ...
  uhppoted-cli delete-all 405419896
  405419896 true
```
...
- `linked profile ID`
```
uhppoted-cli [options] get-time-profile <device ID> <profile ID>
  ...
  uhppoted-cli get-time-profile 405419896 2
  405419896  2 2021-04-01:2021-12-31 Mon,Wed,Fri 08:30-11:30,13:45-17:00 3
  
  uhppoted-cli get-time-profile 405419896 29
...
- `linked profile ID`
```
uhppoted-cli [options] get-time-profile <device ID> <profile ID> <from:to> <weekdays> <segments> <linked>
  ...
  uhppoted-cli set-time-profile 405419896 29 2021-04-01:2021-12-31 3 Mon,Wed,Fri 08:30-11:30,,13:45-17:00 3
  405419896: time profile 29 created
```
**NOTES**
1. `uhppoted-cli` requires the linked profile to be an existing time profile defined in the controller. The controller itself does not enforce this requirement but linking to a non-existent time profile counter-intuitively seems to allow access at any time of day.
2. `uhppoted-cli` does not allow a time profile to define a link that creates a circular chain of time profiles. Again, the controller itself does not enforce this requirement but is indicative of a mistake in the definition of the time profiles.
...
```
uhppoted-cli [options] clear-time-profiles <device ID>
  ...
  uhppoted-cli clear-time-profiles 405419896
  405419896 true
```
**NOTES**
...
```
uhppoted-cli [options] get-time-profiles <device ID> <TSV>
  ...
  uhppoted-cli get-time-profiles 405419896
  ...
  uhppoted-cli get-time-profiles 405419896 405419896.tsv
```
...
```
uhppoted-cli [options] set-time-profiles <device ID> <TSV>
...
101     2021-04-01  2021-10-29  N   Y   N    N    Y   N   Y   08:30   11:30  00:00   03:00 13:45   17:00 102
  ...
  uhppoted-cli set-time-profiles 405419896 405419896.tsv
  
  ./bin/uhppoted-cli set-time-profiles 405419896 405419896.tsv
   ... set time profile 3
   ... set time profile 29
...
2. There is no requirement for the profiles to be in any particular order e.g. `uhppoted-cli` is capable of creating a time profile that is linked to a profile that is defined after it in the TSV file.
...
```
uhppoted-cli [options] clear-task-list <device ID>
  ...
  uhppoted-cli clear-task-list 405419896
  405419896 true
```
...
```
uhppoted-cli [options] refresh-task-list <device ID>
  ...
  uhppoted-cli refresh-task-list 405419896
  405419896 true
```
...
```
uhppoted-cli [options] add-task <device ID> <task> <door> <from:to> <days> <start> <cards>
  ...
  uhppoted-cli add-task 405419896 3 4 2021-01-01:2021-12-31 Mon,Fri 08:30
  405419896 true

  uhppoted-cli add-task 405419896 'enable more cards' 4 2021-01-01:2021-12-31 Mon,Fri 08:30 29
  405419896 true
```
...
```
uhppoted-cli [options] set-task-list <device ID> <file>
  ...
  uhppoted-cli set-task-list 405419896 405419896.tasks

  ./bin/uhppoted-cli set-task-list 405419896 405419896.tasks
...
```
uhppoted-cli [options] get-events <device ID>
  ...
  Examples:
  > uhppoted-cli get-events 405419896
    405419896  1  69 53
  
  > uhppoted-cli get-events 303986753
    303986753  NO EVENTS
```
...
```
uhppoted-cli [options] get-event <device ID> <event ID>
...
```
uhppoted-cli [options] get-event-index <device ID>
  ...
  Examples:
  > uhppoted-cli get-event-index 405419896
    405419896  24
```
...
```
uhppoted-cli [options] set-event-index <device ID> <index>
  ...
  Examples:
  > uhppoted-cli set-event-index 405419896 19
    405419896  19       true
```
...
```
uhppoted-cli [options] open <device ID> <door>
...
```
uhppoted-cli [options] set-pc-control <device ID> <enable>
  ...
  Examples:
  > uhppoted-cli set-pc-control 405419896
    405419896  true

  > uhppoted-cli set-pc-control 405419896 true
    405419896  true

  > uhppoted-cli set-pc-control 405419896 false
    405419896  false
```
...
```
uhppoted-cli [options] set-interlock <controller ID> <interlock>
  ...
  <interlock>     (required) Sets the controller interlock mode. Valid values are:
                  ...
                  e.g. uhppoted-cli set-interlock 405419896 '1&2,3&4'
  ...
  Examples:
  > uhppoted-cli set-interlock 405419896 '1&2,3&4'
    405419896  set interlock 1&2,3&4

  > uhppoted-cli set-interlock 405419896 none
...
```
uhppoted-cli [options] activate-keypads <controller-id> <readers>
  ...
  Examples:
  > uhppoted-cli activate-keypads 405419896 1,2,4
    405419896  activated keypads 1,2,4

  > uhppoted-cli activate-keypads 405419896
...
```
uhppoted-cli [options] set-super-passwords <controller-id> <door> <password>
  ...
  Examples:
  > uhppoted-cli set-super-passwords 405419896 1 12345,999999
    405419896  set super passwords 405419896 1 ok

  > uhppoted-cli set-super-passwords 405419896 1
    405419896  set super passwords 405419896 1 ok

  > uhppoted-cli --debug set-super-passwords Alpha 1 12345,999999,23456,34567,45678,567890
    405419896  set super passwords 405419896 1 ok
```
...
```
uhppoted-cli [options] get-antipassback <controller-id>
  ...
  Examples:
  > uhppoted-cli get-antipassback 405419896
...
```
uhppoted-cli [options] set-antipassback <controller-id> <antipassback>
  ...
  Examples:
  > uhppoted-cli set-antipassback 405419896 "(1:3);(2:4)"
...
```
uhppoted-cli [options] set-firstcard <controller-id> <door> <start> <end> <active> <inactive> <weekdays>
  ...
  Examples:
  > uhppoted-cli set-firstcard 405419896 4
    405419896  4 set first-card ok  

  > uhppoted-cli set-firstcard 405419896 4 08:30 16:45 normally-open normally-closed Mon,Tue,Fri
    405419896  4 set first-card ok
```
...
```
uhppoted-cli [options] restore-default-parameters <controller-id>
  ...
  Examples:
  > uhppoted-cli restore-default-parameters 405419896
...
```
  uhppoted-cli [options] revoke <card> [doors]
  ...
  uhppoted-cli revoke 918273645 Garage
...
```
   uhppoted-cli [options] show <card>
...
```
   uhppoted-cli [options] load-acl [--with-pin] [--strict] [--card-format <any|wiegand-26>] <ACL file>
  ...
  uhppoted-cli --debug --conf warehouse.conf load-acl warehouse.acl
  uhppoted-cli --debug --conf warehouse.conf load-acl --card-format wiegand-26 --with-pin warehouse.acl
...
```
  uhppoted-cli [options] get-acl <file>
  ...
  uhppoted-cli --debug get-acl 2020-05-18.acl
  uhppoted-cli --debug --conf warehouse.conf get-acl --with-pin 2023-03-07.acl
```
...
```
   uhppoted-cli [options] compare-acl <file> <report>
  ...
  uhppoted-cli --debug compare-acl warehouse.acl 2020-05-18.rpt
  uhppoted-cli --debug --conf warehouse.conf compare-acl --with-pin warehouse.acl 2020-05-18.rpt
```
